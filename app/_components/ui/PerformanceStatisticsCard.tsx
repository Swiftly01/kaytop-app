...
export interface PerformanceStatSection {
  label: string;
  branchName: string;
  value: string | number;
  change: number;
...
export interface PerformanceStatisticsCardProps {
  sections: PerformanceStatSection[];
}

export const PerformanceStatisticsCard: React.FC<PerformanceStatisticsCardProps> = ({ sections }) => {
  // Safety check - ensure sections is an array
  if (!Array.isArray(sections)) {
    console.error('PerformanceStatisticsCard: sections prop is not an array:', sections);
    return (
      ...
      {sections.map((section, index) => (
        <React.Fragment key={index}>
          ...
          <div className="flex-1 px-4 py-4 sm:px-6 sm:py-6">
            ...
            >
              {String(extractValue(section.branchName, 'Unknown Branch'))}
            </div>
