...
export default function UnifiedLoginForm({ onSuccess, redirectPath }: UnifiedLoginFormProps) {
  const { loginUnified, isLoading: authLoading } = useAuth();
  const router = useRouter();
  const searchParams = useSearchParams();
  
  const [formData, setFormData] = useState<UnifiedLoginFormData>({
    email: '',
    password: '',
    keepSignedIn: false
  });
  ...
  const handleRealTimeValidation = (field: keyof UnifiedLoginFormData, value: string) => {
    if (value.length > 0) {
  ...
  const handleInputChange = (field: keyof UnifiedLoginFormData) => (
  ...
  useEffect(() => {
    ...
    const emailInput = document.getElementById('unified-email-input');
    if (emailInput) {
      emailInput.focus();
    }
  ...
  useEffect(() => {
    const announceStatus = () => {
      const statusElement = document.getElementById('unified-login-status');
      if (statusElement) {
        if (isLoading) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use unified login credentials - the auth service will handle role detection
      await loginUnified({
      ...
      // The AuthContext will handle role-based routing automatically
  ...
  return (
    <>
      ...
      <form 
        ...
        role="form" 
        aria-labelledby="unified-login-heading"
      >
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div className="relative">
            <input
              ...
              aria-describedby={errors.email ? 'unified-email-error' : undefined}
              disabled={isLoading}
              autoComplete="email"
            />
          </div>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <div className="relative">
            <input
              ...
              aria-describedby={errors.password ? 'unified-password-error' : undefined}
              disabled={isLoading}
        ...
        <div className="flex items-center justify-between" style={{ marginBottom: '32px' }}>
          <div className="flex items-center gap-3">
            ...
            />
            <label 
              htmlFor="unified-keep-signed-in"
              style={{
                fontSize: '14px',
                fontWeight: 400,
                lineHeight: '22px',
